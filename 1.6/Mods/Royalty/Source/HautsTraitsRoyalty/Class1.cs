...
namespace HautsTraitsRoyalty
{
    ...
    public static class HautsTraitsRoyalty
    {
        ...
        public static void HautsTraitsLPLove_AddDirectRelationPostfix(Pawn_RelationsTracker __instance, PawnRelationDef def, Pawn otherPawn)
        {
            ...
            {
                PsychicAwakeningUtility.LPLoveCheckRelations(def,pawn,otherPawn,false);
            }
        }
    ...
    public class Hediff_KeaCuriosity : HediffWithComps
    {
        ...
        public override void PostTickInterval(int delta)
        {
            ...
            if (this.pawn.IsHashIntervalTick(20, delta))
            {
                if (this.pawn.psychicEntropy != null && this.pawn.psychicEntropy.IsCurrentlyMeditating)
                {
                    PsychicAwakeningUtility.KeaOffsetPsyfocusLearning(this.pawn, 0.0004f * this.pawn.GetStatValue(StatDefOf.MeditationFocusGain), 0.02f);
                }
            }
        }
        public override void Notify_PawnUsedVerb(Verb verb, LocalTargetInfo target)
        {
            ...
            } else if (verb is VEF.Abilities.Verb_CastAbility vcavfe && HautsUtility.IsVPEPsycast(vcavfe.ability)) {
        ...
        public override void Notify_PawnDied(DamageInfo? dinfo, Hediff culprit = null)
        {
            ...
            if (PsychicAwakeningUtility.ShouldTranscend(pawn) && pawnToRez.story != null && !pawnToRez.story.traits.HasTrait(HVTRoyaltyDefOf.HVT_TTraitWraith))
            {
                if (Rand.Value <= 0.05f)
                {
                    ...
                    if (ResurrectionUtility.TryResurrect(pawnToRez))
                    {
                        PsychicAwakeningUtility.AchieveTranscendence(pawnToRez, "HVT_TransDeath".Translate().CapitalizeFirst().Formatted(pawnToRez.Named("PAWN")).AdjustedFor(pawnToRez, "PAWN", true).Resolve(), "HVT_TransDeathFantasy".Translate().CapitalizeFirst().Formatted(pawnToRez.Named("PAWN")).AdjustedFor(pawnToRez, "PAWN", true).Resolve(), 0.2f, false, ignoreGeneChanceSetting: true);
                    }
                    ...
                    if (archonexi.Count > 0)
                    {
                        if (ResurrectionUtility.TryResurrect(pawnToRez))
                        {
                            PsychicAwakeningUtility.AchieveTranscendence(pawnToRez, "HVT_TransArchoDeath".Translate().CapitalizeFirst().Formatted(pawnToRez.Named("PAWN")).AdjustedFor(pawnToRez, "PAWN", true).Resolve(), "HVT_TransArchoDeathFantasy".Translate().CapitalizeFirst().Formatted(pawnToRez.Named("PAWN")).AdjustedFor(pawnToRez, "PAWN", true).Resolve(), 1f, false, ignoreGeneChanceSetting: true);
                        }
                    }
                }
            }
    ...
    public class HediffComp_MoteTranscendent : HediffComp_MoteConditional
    {
        ...
        {
            base.CompPostTickInterval(ref severityAdjustment, delta);
            if (this.Pawn.IsHashIntervalTick(2500, delta) && this.Props.mustBeTranscendent && !PsychicAwakeningUtility.IsTranscendent(this.Pawn))
            {
    ...
    public class CompAbilityEffect_Syzygy : CompAbilityEffect_ForcedByOtherProperty
    {
        ...
        {
            if (this.parent.CanCast && !this.parent.pawn.IsPlayerControlled && this.parent.pawn.CurJob != null && ((this.parent.pawn.CurJob.ability != null && this.parent.pawn.CurJob.ability.def.IsPsycast) || (this.parent.pawn.CurJob.verbToUse is VEF.Abilities.Verb_CastAbility vca && HautsUtility.IsVPEPsycast(vca.ability))))
            {
    ...
    public static class PsychicAwakeningUtility
    {
        ...
        {
            if (pawn.story != null && (IsAwakenedPsychic(pawn, false) || (IsAwakenedPsychic(pawn) && (ignoreGeneChanceSetting || Rand.Chance(HVT_Mod.settings.wokeGeneTransSuccessChance)))))
            {
