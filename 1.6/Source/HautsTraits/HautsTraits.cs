...
namespace HautsTraits
{
    ...
    public class Hediff_Conform : HediffWithComps
    {
        ...
        {
            base.PostTickInterval(delta);
            if (this.pawn.IsHashIntervalTick(120000,delta) && this.pawn.Faction != null && ModsConfig.IdeologyActive && !this.pawn.IsMutant)
            {
            ...
            if (floatMenuAcceptanceReport.Accepted || !floatMenuAcceptanceReport.FailReason.NullOrEmpty() || !floatMenuAcceptanceReport.FailMessage.NullOrEmpty())
            {
                if (!floatMenuAcceptanceReport.FailReason.NullOrEmpty())
                {
                    ...
                    Action action = delegate
                    {
                        ...
                        {
                            caravan.pather.StartPath(site.Tile, HVTUtility.GoToScoutForAmbushes(caravan,site), true, true);
                            return;
                        }
